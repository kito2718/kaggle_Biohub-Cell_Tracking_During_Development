...
1. **速度ベクトル（慣性）の導入 (Velocity-aware Matching)**:
   - 現在は「直前の位置 $x_t$」と「次の位置 $x_{t+1}$」の距離を測っています。
   - 直前フレームの移動量 $\vec{v}_t = x_t - x_{t-1}$ を使って予測位置 $\hat{x}_{t+1} = x_t + \vec{v}_t$ を求め、この予測点からの距離をコストとすることで、流動する胚組織の追跡精度が劇的に向上します。
2. **空間距離 vs 特徴量重みの最適化 (`spatial_weight` vs `feature_weight`)**:
   - 現在の `1.0 vs 0.5` を、代表データセットを用いて `1.0 vs 0.2〜0.8` の範囲で高速グリッドサーチし、最良のバランスを見出します。
3. **トラッキング後処理による「孤立ノードの刈り取り (P/E比 0.90 誘導)」**:
   - リンクを一切形成できなかった長さ1の孤立ノード（時間的一貫性のない微小ゴミ）をトラッキング段階で除外することで、Recall 91.9% を保ちながら P/E比を 1.15 $\to$ 0.90〜0.95 へ完全着地させます。
...
3. **エッジ検出（トラッキング）の確定方針**:
   ...
   - 今後のエッジ改善策は、チューニング難度の高い `btrack` の深追いを止め、**「5D Mahalanobis の調整・高度化（速度慣性ベクトル導入 ＋ 孤立ノード刈り取り）」に全リソースを集中投入するのが最も勝率の高い王道路線**となります。
