...
### 12.5 次のステップ: 単一事前学習済み LightGBM モデルによるエッジ追跡の飛躍的精度向上

#### 1. なぜ機械学習モデルなのか？ (マハラノビス法および深層学習GNNとの比較)

トラッキング問題において、現在採用しているマハラノビス距離と、他の有力候補(深層学習GNN、機械学習決定木)を比較検討した結果、Kaggleコンペティションの実行制約・精度最大化の観点から **ペアワイズ二値分類モデルが最も強力かつ現実的** であると結論付けられました。

| 手法体系 | 表現力 (非線形性) | 計算コスト (推論速度) | メモリ・GPU依存度 | 備考・採否 |
| :--- | :---: | :---: | :---: | :--- |
| **現行 4D Mahalanobis** | 低 (線形距離) | **極小 (< 0.1ms/frame)** | 完全ゼロ (CPUのみ) | 安定しているが、特徴量間の条件付き分岐(例: 距離が近ければ輝度差を許容、遠ければ厳格判定)が表現不能。 |
| **深層学習 (GNN / Transformer)** | 極高 (軌跡全体の文脈) | 極大 (数秒〜十数秒/frame) | 大 (GPU必須、OOMリスク) | 19,417フレームの全量推論ではKaggleの9時間制限を超過するリスク大。学習データ規模に対して過学習の危険。 |
| **事前学習済み決定木アンサンブル (推奨)** | **高 (決定木の条件付き非線形)** | **極小 (< 1〜2ms/frame)** | **極小 (CPUのみで瞬時推論)** | **最適解**。物理空間距離や4Dマハラノビス距離自体を入力特徴量として内包できるため、**現行精度(67.5%)を理論的に下回ることがない**。 |

#### 2. 他の決定木系(枝系)モジュールとの比較: なぜ LightGBM を選択するのか？

決定木・アンサンブル学習(枝系)には、LightGBM 以外にも **XGBoost**, **CatBoost**, **Random Forest (scikit-learn)**, **HistGradientBoosting** などの有力なモジュールが存在します。その中で本パイプラインに **LightGBM を選定する明確な技術的根拠** を下表および詳細項目にまとめます。

| アルゴリズム / モジュール | 分割戦略 (Tree Growth) | 推論速度 (CPU) | モデルサイズ | トラッキング不均衡データへの適合性 | 採否理由 |
| :--- | :--- | :---: | :---: | :---: | :--- |
| **LightGBM (Microsoft)** | **Leaf-wise (葉単位・最良優先)** | **超高速 (最速)** | **極小 (~200KB-1MB)** | **極めて高い (不均衡・局所スパースに最強)** | **【採用】** 推論速度が圧倒的で、非対称な細胞分裂・移動判定を最小の木で学習可能。 |
| **XGBoost (DMLC)** | Depth-wise (深さ単位) | 高速 (LightGBMの1/2〜1/3) | 小 (~2MB-5MB) | 高い | 良好だが、CPU推論速度およびメモリ消費において LightGBM に一歩及ばない。 |
| **CatBoost (Yandex)** | Symmetric Tree (対称木・固定深さ) | 高速 | 中 (~5MB-10MB) | 中 (カテゴリ変数向け) | カテゴリ変数に強いが、本タスクは全て連続数値(比率・変位)。対称木は非対称な境界の表現に多くの深さを要する。 |
| **Random Forest (sklearn)** | Depth-wise (バギング・深さ無制限) | 低速 (LightGBMの1/10〜1/30) | **極大 (100MB-500MB超)** | 低い (過学習・メモリ爆発) | ノートブック提出時にモデル容量が巨大化し、大量ペアのCPU推論で大幅なボトルネックとなる。 |

##### 【選定理由の詳細】
1. **Leaf-wise (葉単位) 分割による高効率な非対称境界の学習**:
   - トラッキングペア分類は、候補ペアのうち正例が約20%という「不均衡データ」です。
   - XGBoost の標準である Depth-wise や Random Forest の均等深さ分割では、全体を均等に掘り下げるため無駄な分岐が大量に発生します。
   - LightGBM の **Leaf-wise (最良優先探索)** は、損失(Loss)の減少が最も大きい葉だけを非対称に深く伸ばすため、「99%確実にリンクする超近距離ペア」や「SNRが急変した分裂細胞」といった**スパースな局所例外ルールを最小の木サイズで的確に分離**できます。
2. **提出環境(19,417フレーム)に耐えうる圧倒的なCPU推論速度**:
   - 本コンペティションの全199データセットには合計 19,417 フレームが存在し、評価・提出時には累積 20万〜50万ペアのスコアリングが発生します。
   - LightGBM は C++ レベルで極限まで最適化されたポインタ分岐により、CPU上でのバッチ推論が **Random Forest の 10〜30倍、XGBoost の 2〜3倍高速** です。全データセット推論でもわずか数秒〜十数秒で完了し、Kaggleノートブックの実行時間を圧迫しません。
3. **モデルサイズが極小 (わずか数百KB) で可搬性が最高**:
   - Random Forest は数百本の深い木をそのまま保持するため、モデルファイルが数百MBに達し、Kaggle環境へのアップロードやメモリ展開の負担となります。
   - LightGBM は 31 leaves $\times$ 50〜100 trees 程度で最高性能に達するため、モデルファイルが **わずか 200KB〜1MB 程度** で済みます。必要であればテキスト形式(`model.txt`)でノートブックのコードセル内に直接埋め込むことすら可能です。
4. **ヒストグラムビン化(Histogram-based Binning)による外れ値耐性**:
   - 体積比率やSNR比率などの「比率特徴量」は、分母が微小値のときに極端なスパイク(外れ値)を示します。
   - LightGBM は連続値を 255 個の離散ヒストグラムビンに圧縮して閾値判定するため、外れ値に対するクリッピング処理などを入念に行わなくても極めて安定した決定境界を形成できます。
5. **Kaggle公式カーネルにおける完全標準バンドル**:
   - Kaggle の Python 環境に最初から組み込まれており、外部ライブラリの追加インストールやバージョン不整合のリスクがゼロです。

#### 3. 「事前学習済み単一モデル」を全データセットで流用する設計優位性
- **学習はオフラインで1回のみ**:
  全199データセットの真のエッジ(128,883本)および近傍誤結合ペアから、共通の二値分類モデル `s4_004_tracking_edge_lgbm.joblib` をローカル環境で1度だけ事前学習します。
- **本番ノートブック(推論環境)の負荷ゼロ**:
  提出用ノートブック(`s3_100_results_integration_and_submission.ipynb`)内では、事前に生成したモデルファイルを `joblib.load()` で読み込み、フレーム間の候補ペアに対して `predict_proba()` を実行するだけです。フレームあたりわずか数ミリ秒で接続確率 $P(\text{link})$ が求まり、コスト行列 $C_{ij} = -\log P(\text{link})$ を既存のハンガリアン法に渡すだけで完了します。

#### 4. 教師データの作成構想 (正例・負例・特徴量エンジニアリング)
- **正例 (Label = 1)**:
  全199データセットの全GTエッジ (128,883本)。
- **負例 (Label = 0)**:
  各フレームの候補細胞 $i$ に対し、次フレーム $t+1$ 内の $d \le 7.0\,\mu\text{m}$ の球体内に存在する細胞 $j$ のうち、GTエッジ以外の近傍誤結合ペア(Hard Negatives)。
- **ペアワイズ特徴量 (約15〜20次元)**:
  1. 3D空間変位: $\Delta z, \Delta y, \Delta x$, 3Dユークリッド距離 $d_{3D}$, XY平面距離 $d_{xy}$, $|\Delta z|$
  2. 光学・形態特徴量の比率・差分:
     - SNR比率 $\text{snr}_1 / (\text{snr}_2 + \epsilon)$, 差分 $|\text{snr}_1 - \text{snr}_2|$
     - 輝度比率 $\text{mean\_intensity}_1 / (\text{mean\_intensity}_2 + \epsilon)$, 差分
     - 体積比率 $\text{volume\_um3}_1 / (\text{volume\_um3}_2 + \epsilon)$, 差分
     - 半径比率 $\text{estimated\_radius\_um}_1 / (\text{estimated\_radius\_um}_2 + \epsilon)$, 差分
  3. ベースライン距離:
     - 4D Mahalanobis 距離 (本検証で最高精度を出した最適スコア)
  4. 局所ランキング特徴量:
     - $i$ から見た近傍候補の中での距離順位(Rank 1st, 2nd, 3rd...)、逆方向 $j$ から見た距離順位

---

### 12.6 生成された納品成果物一覧

すべて `s3_results_integration_and_submission/__履歴__/` 配下に格納されています：
- スクリプト: 
  - `s4_003_feature_ablation_5dmahalanobis_all199.py`
- 集計データ:
  - `s4_003_feature_ablation_all199.csv`
  - `s4_003_feature_ablation_all199.xlsx`
- プロット図:
  - `s4_003_feature_ablation_leave_one_out.png` (特徴量重要度ランキング棒グラフ)
  - `s4_003_feature_ablation_all_configs.png` (全12構成の精度比較グラフ)
- 総合レポート:
  - `s4_999_pe_ratio_control_report.md` (第12章アブレーション結果および第13章まとめを完全反映)
