...
# =============================================================================
# s7_034_d8_volumetric_tta.py
# Biohub SOTA 0.952+ (3D Volumetric TTA with D8 Symmetry)
# =============================================================================
...
_new = """        if cfg.det_tta:
            ...
            del imgs_at, det_at
            _nv += 1
            # 3D Depth Inversion (Z-axis volumetric symmetry)
            imgs_z = imgs.flip((-3,))
            _, det_z = model.encode(imgs_z)
            for f in range(W):
                det_logits[f] = det_logits[f] + det_z[f].flip((-3,))
            del imgs_z, det_z
            _nv += 1
            # 3D Full Central Inversion (Z, Y, X symmetry)
            imgs_xyz = imgs.flip((-3, -2, -1))
            _, det_xyz = model.encode(imgs_xyz)
            for f in range(W):
                det_logits[f] = det_logits[f] + det_xyz[f].flip((-3, -2, -1))
            del imgs_xyz, det_xyz
            _nv += 1
            for f in range(W):
