...
def predict_video(
    ...
    for ws in tqdm(
        ...
        if cfg.det_tta:
            ...
            # 3D Depth Inversion
            imgs_z = imgs.flip((-3,))
            _u_z, det_z = model.encode(imgs_z)
            for f in range(W):
                det_logits[f] = det_logits[f] + det_z[f].flip((-3,))
            if _edge_tta:
                _unet_acc = _unet_acc + _u_z.flip((-3,))
            del imgs_z, det_z, _u_z
            _nv += 1
            # 3D Full Central Inversion
            imgs_xyz = imgs.flip((-3, -2, -1))
            _u_xyz, det_xyz = model.encode(imgs_xyz)
            for f in range(W):
                det_logits[f] = det_logits[f] + det_xyz[f].flip((-3, -2, -1))
            if _edge_tta:
                _unet_acc = _unet_acc + _u_xyz.flip((-3, -2, -1))
            del imgs_xyz, det_xyz, _u_xyz
            _nv += 1
            for f in range(W):
            ...
            if _edge_tta:
                ...
                if _delta == 0.0:
                    raise RuntimeError('EDGE-TTA NO-OP')
