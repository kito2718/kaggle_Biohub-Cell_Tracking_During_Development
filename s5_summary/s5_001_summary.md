...
### 4.4 Step 2: LightGBM 教師データ (細胞ペア特徴量) 生成完了とエビデンス
...
- **入力元データ**:
  - BlobDog 検出ノード: `s3_01_detect_nodes_pred_blobdog_5dmahalanobis_*.csv` (代表5データセット 110,883 件)
  - GT 正解データ: `s3_gt_nodes.csv` (2,302 件), `s3_gt_edges.csv` (2,251 本)
- **生成スクリプト**: `s5/github/s5_analysys_data/s5_002_generate_lgbm_pairs.py`
- **検証スクリプト**: `s5/github/s5_analysys_data/s5_002_test_pair_dataset.py`
- **生成成果物**:
  - Parquet (本番学習用): `s5/github/s5_analysys_data/s5_002_train_pairs_5datasets.parquet` (11.96 MB, 142,005 行, 30 列)
  - サンプル CSV: `s5/github/s5_analysys_data/s5_002_train_pairs_sample.csv` (先頭 3,000 行)
  - エビデンス CSV: `s5/github/s5_analysys_data/s5_002_train_pairs_evidence.csv`

#### データセット別ペア生成実績 (`s5_002_train_pairs_evidence.csv`):
| データセット | 検出ノード数 | GTノード数 | GTエッジ数 | TPマッチノード | ノードRecall | 正例ペア数 | 負例ペア数 | 負例比率 | 総ペア数 | 所要時間 |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| `44b6_0113de3b` | 34,247 | 52 | 50 | 52 | 100.0% | 47 | 53,364 | 1:1135.4 | 53,411 | 25.9秒 |
| `44b6_0b24845f` | 39,552 | 51 | 49 | 35 | 68.6% | 28 | 45,118 | 1:1611.4 | 45,146 | 22.2秒 |
| `44b6_74d0c52e` | 16,787 | 140 | 139 | 123 | 87.9% | 105 | 18,512 | 1:176.3 | 18,617 | 9.3秒 |
| `6bba_05b6850b` | 9,548 | 861 | 845 | 794 | 92.2% | 709 | 11,601 | 1:16.4 | 12,310 | 6.3秒 |
| `6bba_085bf656` | 10,749 | 1,198 | 1,168 | 1,187 | 99.1% | 1,037 | 11,484 | 1:11.1 | 12,521 | 6.4秒 |
| **合計 / 全体** | **110,883** | **2,302** | **2,251** | **2,191** | **95.2%** | **1,926** | **140,079** | **1:72.7** | **142,005** | **79.2秒** |

#### 主要特徴量の正例 vs 負例の分離度 (実測平均値):
- `spatial_dist` (物理空間距離): 正例 **2.63 μm** vs 負例 **3.35 μm** (正例が有意に近接)
- `mahalanobis_dist` (4Dマハラノビス距離): 正例 **0.3514** vs 負例 **0.9057** (負例が **2.58倍** 大きい)
- `total_cost_4d` (現行トラッカー総合コスト): 正例 **2.98** vs 負例 **4.25** (明瞭に分離)
- `snr_diff` (SNR差分): 正例 **0.2290** vs 負例 **0.5986** (負例が **2.61倍** 変動)
- `volume_diff` (体積差分): 正例 **1.21 μm³** vs 負例 **3.42 μm³** (負例が **2.83倍** 乖離)
- `spatial_rank` (距離順位): 正例 **1.14位** vs 負例 **1.36位** (正例の約88%が第1位候補)
- **データ品質検証**: 全30列で NaN 0件, inf 0件 (**ALL PASS**)
