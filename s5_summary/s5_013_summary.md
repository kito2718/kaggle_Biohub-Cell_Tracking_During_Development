...
- **実走テスト (`s5_010`) 結果**:
  - 実画像 `44b6_0113de3b.zarr` に対し、Pure GPU DoG 検出で **0.130 秒/frame (CPU比 5.70 倍速)** を記録。
  - シグナル特徴量抽出 ➔ 動的閾値予測 (所要時間 **1.0 ms**) ➔ LightGBM エッジ追跡 (0.047 秒) ➔ 孤立ノード刈り取りまで **エラーゼロで完全完走** を確認。
