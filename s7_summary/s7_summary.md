...
### (3) 048 (究極統合 SOTA) の具体方針
1. **アタック 1: 細胞分裂候補探索と Division Jaccard の初獲得**:
   - 母細胞 1 個に対して $t+1$ の娘細胞 2 個が結合する分岐エッジ ($1 \to 2$) を導入。
   - Division Jaccard を 0.000 から 0.30〜0.50 へ引き上げ、スコアへ **`+0.030〜+0.050` の直接ボーナスを加算** する。
2. **アタック 2: 3D 形態特徴量 (045) による物理的分裂バリデーション**:
   - 細胞分裂における物理保存則：母細胞体積 $V_{\text{mother}} \approx V_{\text{daughter1}} + V_{\text{daughter2}}$、および娘細胞同士の空間的対称性を 045 形態パイプラインで検証し、偽分裂を排除。
3. **アタック 3: 047 v2 最強基盤へのダイレクト統合**:
   - 8-View D4 TTA (041b) ＋ エルミートスプライン ＋ 運動学的適応ガード (047 v2) の上に、分裂検出モジュールをマウント。
