...
### (5) 【最重要反省事項】「全上書きバグ」の教訓と今後の再発防止ルール (Anti-Regression Rules)
今回の 043b (0.910) / 047 v2 (0.910) の暴落は、今後のコンペティションおよびアルゴリズム開発において極めて貴重な教訓を残した。二度と同じ失敗を繰り返さないため、以下の再発防止ルールをプロジェクト憲章として厳格に定義する。

1. **「リファイン (微修正)」と「リプレイス (全置換)」の厳格な峻別**:
   - 後処理で「リファイン」と銘打つコードを書く場合、`edges = refined_edges` のような全代入は厳禁とする。
   - 必ず `set(edges_old) & set(edges_new)` によるエッジ維持率を監査し、既存の優良エッジが 95% 以上保持されていることを数学的に証明しなければならない。維持率が 50% を切るような処理は「リファイン」ではなく「全破壊」である。
2. **エッジ追加処理における「加法性 (Additive)」の原則**:
   - スプライン補間やギャップ補完など、エッジを救済する処理は、既存の高品質エッジリストに対して追加する形式 (`edges = [*edges, *new_gap_edges]`) を徹底し、元リストを別配列で丸ごと上書きしてはならない。
3. **ローカル GT 4 胚ベンチマークの「盲点 (死角)」に対する警戒**:
   - 手元の 4 胚ベンチマークは、2 胚が超疎 (0.5 cells/frame)、1 胚が 13 細胞しか正解アノテーションがないため、単純なユークリッド距離マッチングでも見かけ上高スコア (0.89814) が出てしまうという「検証機の死角」が存在した。
   - 一方、本番テスト胚には細胞が交差・密集する難所が大量に存在する。**「50 エポック 3D U-Net が予測した学習済みエッジ確率 (`learned_edge_probs`) を捨てて生座標だけで解く幾何処理」は、ローカルでどれほど良く見えても本番 LB では確実に壊滅する** という事実を銘記する。
